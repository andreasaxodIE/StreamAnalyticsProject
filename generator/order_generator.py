...
Produces realistic, configurable sequences of order status-change events
with intentional edge cases: out-of-order delivery, duplicates, missing
steps, impossible durations, and anomalous cancellations.
"""
...
from config import (
    ...
    PROMO_CODES, weighted_choice, zone_weighted_choice, hourly_demand_weight,
)
...
def build_restaurants(n: int) -> List[Dict]:
    ...
    for i in range(n):
        ...
        restaurants.append({
            ...
            "cuisine": cuisine,
            "zone_id": zone_id,
            "avg_prep_time_seconds": random.randint(600, 1800),  # 10–30 min
...
def _inter_event_delay(from_status: str, to_status: str, anomalous: bool) -> int:
    ...
    if anomalous:
        # "impossible" durations: either too fast or preposterously long
        choice = random.random()
        if choice < 0.5:
            return random.randint(1, 5)       # suspiciously fast
        else:
            return random.randint(7200, 18000)  # 2–5 hours
...
class OrderEventGenerator:
    ...
    def _make_base_event(
        ...
        estimated_prep: Optional[int],
        estimated_delivery: Optional[int],
        actual_prep: Optional[int],
        actual_delivery: Optional[int],
    ...
    def generate_order_lifecycle(self, base_ts_ms: int) -> List[Dict]:
        """Generate all events for a single order."""
        cfg = self.cfg
        ...
        if random.random() < cfg.promo_order_rate:
            promo_code, discount_pct = random.choice(PROMO_CODES)
        discount_cents = int(total * discount_pct / 100)
        delivery_fee = random.randint(99, 499)
        ...
        if will_cancel:
            # Cancel at a random early stage
            cancel_at = random.randint(1, 4)
            lifecycle = lifecycle[:cancel_at] + ["CANCELLED"]
        elif skip_step:
            # Remove one intermediate step (e.g. jump from READY_FOR_PICKUP to EN_ROUTE)
            removable = [2, 3, 4]  # PREPARING, READY_FOR_PICKUP, PICKED_UP
        ...
        for i, status in enumerate(lifecycle):
            ...
            # Late arrival injection
            is_late = random.random() < cfg.late_arrival_rate
            if is_late:
                late_delay_ms = random.randint(30_000, cfg.max_late_arrival_seconds * 1000)
                ingestion_ts = current_ts + late_delay_ms
            else:
                ingestion_ts = current_ts + random.randint(50, 2000)  # normal ~0.05-2s lag
            ...
            # Only put items/total on PLACED
            ev_items = items if status == "PLACED" else None
            ev_total = (total - discount_cents) if status == "PLACED" else None
            ev_delivery_fee = delivery_fee if status == "PLACED" else None

            est_prep = restaurant["avg_prep_time_seconds"] if status in ("PLACED", "ACCEPTED") else None
            est_delivery = (restaurant["avg_prep_time_seconds"] + 900) if status == "PLACED" else None
            ...
            event = self._make_base_event(
                ...
                promo_code=promo_code if status == "PLACED" else None,
                discount_cents=discount_cents if status == "PLACED" else None,
                delivery_fee_cents=ev_delivery_fee,
            ...
            if courier_vanish and status == "PICKED_UP":
                # Next event will show courier offline — emit an out-of-order
        ...
        # Inject rating on successful delivery
        if lifecycle[-1] == "DELIVERED" and random.random() < 0.65:
            rating_event = copy.deepcopy(events[-1])
            rating_event["event_id"] = str(uuid.uuid4())
            rating_event["order_status"] = "DELIVERED"
            rating_event["customer_rating"] = round(random.gauss(4.1, 0.8), 1)
            rating_event["customer_rating"] = max(1.0, min(5.0, rating_event["customer_rating"]))
            rating_event["event_timestamp"] += random.randint(60_000, 3_600_000)
            rating_event["is_late_arrival"] = True  # ratings often arrive well after delivery
    ...
    def stream(self, start_ts_ms: int, n_orders: int) -> Iterator[Dict]:
        """Yield events for n_orders in approximate chronological order."""
        all_events = []
        for _ in range(n_orders):
            # Stagger order placement across time window
        ...
        # Sort by ingestion_timestamp to simulate broker arrival order
