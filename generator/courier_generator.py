...
def _move_towards(lat, lon, target_lat, target_lon, speed_kmh, seconds):
    ...
    return round(lat + frac * (target_lat - lat), 6), round(lon + frac * (target_lon - lon), 6)
...
class CourierSimulator:
    ...
    def _make_event(self, ts_ms: int, status: str,
                    anomaly_flag=None, is_duplicate=False, is_late=False,
                    distance_to_restaurant=None, distance_to_customer=None) -> Dict:
    ...
    def generate_shift(self, start_ts_ms: int, duration_seconds: int) -> List[Dict]:
        ...
        while ts < end_ts:
            ...
            events.append(self._make_event(ts, "HEADING_TO_RESTAURANT", distance_to_restaurant=dist_to_rest_m))
            ...
            if random.random() < cfg.courier_offline_mid_delivery_rate:
                ...
                events.append(self._make_event(ts, "OFFLINE", anomaly_flag="OFFLINE_MID_DELIVERY"))
                self.order_id = None
            ...
            while ping_ts < ts + delivery_time * 1000:
                ...
                is_late = random.random() < cfg.late_arrival_rate
                events.append(self._make_event(ping_ts, "EN_ROUTE_TO_CUSTOMER",
